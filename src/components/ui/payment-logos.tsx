...
/**
 * Las marcas de los medios de pago.
 *
 * Van dibujadas como SVG y no como imagen a propósito: son las dos únicas
 * marcas que no son nuestras dentro de la app y aparecen en el momento más
 * delicado de todo el recorrido —cuando alguien está por conectar su cuenta de
 * cobros—, así que tienen que verse nítidas en cualquier tamaño y cargar con la
 * página, sin un salto de layout justo ahí.
 *
 * Un emoji de bandera y una tarjetita genérica, que es lo que había antes, no
 * transmiten lo mismo: la marca conocida es buena parte de la confianza que
 * hace falta para entregar los datos de la cuenta donde va a entrar la plata.
 *
 * Los colores son los de cada marca y no se tocan con el tema de la app: un
 * logo recoloreado deja de ser el logo. Por eso tampoco cambian en modo oscuro.
 *
 * ── Procedencia, que acá importa ─────────────────────────────────────────────
 * Mercado Pago son los trazos del archivo oficial de la marca, sin retocar. El
 * archivo completo —el mismo dibujo con el logotipo escrito debajo— queda
 * guardado en `public/brand/mercadopago-vertical.svg` para cualquier otro uso.
 *
 * Stripe todavía es una reproducción hecha a mano. Si aparece el archivo
 * oficial, hay que reemplazarla: un logo dibujado de memoria se parece, y
 * parecerse no alcanza cuando la marca es de otro.
 */

/**
 * Los colores exactos de Mercado Pago, tal como vienen en el archivo oficial.
 * No son aproximaciones: si alguno cambia, deja de ser el logo.
 */
const MP_AMARILLO = "#FFE600";
const MP_CELESTE = "#00BCFF";
const MP_AZUL = "#0A0080";

interface LogoProps {
  className?: string;
  /** Lado del cuadrado en píxeles. Los dos marcos son cuadrados y del mismo alto. */
  size?: number;
}

/**
 * Mercado Pago — el apretón de manos dentro de la elipse.
 *
 * Es el isotipo, sin el texto: al lado ya va el nombre escrito, y meter también
 * las dos palabras adentro del cuadrado obligaría a achicar el dibujo hasta que
 * no se lea ninguna de las dos cosas.
 */
export function MercadoPagoLogo({ className, size = 44 }: LogoProps) {
  return (
    <span
      className={cn(
      ...
      style={{ width: size, height: size, backgroundColor: MP_AMARILLO }}
    >
      {/*
        El `viewBox` recorta el archivo oficial, que viene de 700×700 con el
        logotipo escrito debajo del apretón de manos. Estos números son los del
        dibujo original: encuadran el isotipo y nada más.

        Los trazos de abajo no se tocan. Moverlos para "centrarlo mejor" o
        "engrosarlo un poco" es redibujar un logo ajeno, que es exactamente lo
        que veníamos haciendo antes de tener el archivo.
      */}
      <svg
        viewBox="205 133 290 203"
        width={size * 0.94}
        height={size * 0.94 * (203 / 290)}
        role="img"
      ...
      >
        ...
        {/* Las manos, en tres piezas: los dos antebrazos y los dedos. */}
        <path
          fill="#FFFFFF"
        ...
        {/* El contorno azul: es lo que hace que el logo se lea a 44 píxeles. */}
        <path
          fill={MP_AZUL}
...
/**
 * Stripe — la marca cuadrada.
 *
 * Es la que Stripe usa como ícono de aplicación: el violeta de la marca con la
 * ese en blanco. Va esta y no el logotipo escrito porque tiene que convivir en
 * la misma grilla que Mercado Pago, y un rectángulo largo al lado de un
 * cuadrado desalinea las dos tarjetas.
 *
 * A diferencia de la de al lado, esta ese está dibujada a mano. Se parece, pero
 * no sale del archivo de Stripe: si conseguís el oficial, reemplazala.
 */
export function StripeLogo({ className, size = 44 }: LogoProps) {
  return (
    <span
      className={cn(
        "inline-flex shrink-0 items-center justify-center overflow-hidden rounded-2xl",
    ...
    >
      ...
      >
        <path
          d="M14.9 12.4c0-1.3 1.1-1.8 2.8-1.8 2.5 0 5.7.8 8.2 2.2V5.1C23.2 4 20.5 3.6 17.7 3.6c-6.7 0-11.2 3.5-11.2 9.4 0 9.2 12.6 7.7 12.6 11.7 0 1.5-1.3 2-3.1 2-2.7 0-6.3-1.1-9.1-2.7v7.8c3.1 1.3 6.2 1.9 9.1 1.9 6.9 0 11.6-3.4 11.6-9.4 0-9.9-12.7-8.1-12.7-11.9Z"
          fill="#FFFFFF"
        />
      </svg>
    </span>
...
/** El logo que corresponde a cada proveedor, por su id. */
export function PaymentLogo({
  ...
  if (provider === "mercadopago") return <MercadoPagoLogo size={size} className={className} />;
  if (provider === "stripe") return <StripeLogo size={size} className={className} />;
  return null;
}
